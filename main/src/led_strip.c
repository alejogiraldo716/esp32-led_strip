...
#define LED_STRIP_TASK_PRIORITY         (configMAX_PRIORITIES - 6)
...
{
    printf("led_strip_init_rmt started\n");
...
bool led_strip_init(struct led_strip_t *led_strip)
{
    ...
    if ((led_strip == NULL) ||
        ...
        printf("led_strip_init fail [1]\n");
        return false;
    }

    if(led_strip->led_strip_buf_1 == led_strip->led_strip_buf_2) {
        printf("led_strip_init fail [2]\n");
        return false;
    }
    ...
    if (!init_rmt) {
        printf("led_strip_init_rmt fail [3]\n");
        return false;
    }
